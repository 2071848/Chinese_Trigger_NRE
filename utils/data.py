...
class Data:
    def __init__(self): 
        ...
        self.ma_ga= 0.5
        self.bert_flag = True
        self.trigger_train = True
        self.punishment_train = False
    ...
    def show_data_summary(self):
        ...
        print("     Hyperpara    use_ma_ga: %s" % (self.ma_ga))
        print("     Hyperpara    use_trigger_train: %s" % (self.trigger_train))
        print("     Hyperpara    use_punishment_train: %s" % (self.punishment_train))
        print("     Hyperpara    bert flag: %s" % (self.bert_flag))
        if self.HP_use_char:
    ...
    def batch_iter(self, name,
        ...
        for batch_id in range(total_batch):
            ...
            trigger_ids_tensor = autograd.Variable(torch.zeros((batch_size, self.MAX_SENTENCE_LENGTH+3)),volatile =  volatile_flag).float()
            ...
            for bid,bag in enumerate(instance):
                ...
                for ins in bag:
                    ...
                    seq, biseq, chr, gaz, label, pos1, pos2,sentence,trigger, ent1_id, ent2_id, mark, omit_type = ins
                    seqlen = len(seq)
                    ...
                    sent_id = torch.LongTensor(tokenizer.encode(sentence))  # 编码
                    ...
                    trigger_ids_tensor[bid] = torch.FloatTensor(bag[0][-5])
                    ent1_ids.append(ent1_id)
                    ent2_ids.append(ent2_id)
                    marks.append(mark)
                    omit_types.append(omit_type)
            ...
            yield (gaz_list, word_seq_tensor, biword_seq_tensor, word_seq_lengths, char_seq_tensor, char_seq_lengths, char_seq_recover, pos1_seq_tensor, pos2_seq_tensor, ins_label_tensor, batch_label_tensor, mask, scope,trigger_ids_tensor, ent1_ids, ent2_ids,marks, omit_types,sent_ids_tensor)
