...
class BiLstmEncoder(nn.Module):
    ...
    def get_seq_features(self, gaz_list, word_inputs, biword_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover, pos1_inputs, pos2_inputs,sent_ids):
        ...
        # print("char_equal_embeds:", char_equal_embeds.shape)#([1, 224, 768])
        # char_equal_embeds_2 = F.relu(self.linear(char_equal_embeds))
        # char_seq_embeds = self.linear(char_seq_embeds)
        # print("char_seq_embeds shape:", char_seq_embeds.shape)#torch.Size([1, 226, 100])

        word_embs = self.word_embeddings(word_inputs)
        # word_embs_2 = F.tanh(self.linear(word_embs))
        if self.bert_flag:
