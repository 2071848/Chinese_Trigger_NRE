...
def evaluate_forward(data, model, name):
    ...
    for batch in data.batch_iter(name,batch_size,False):
        gaz_list, batch_word, batch_biword, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_pos1, batch_pos2, ins_label, batch_label, mask, scope, trigger_ids, ent1_ids, ent2_ids,marks, omit_types,sent_ids = batch
        prob,alpha = model(gaz_list, batch_word, batch_biword, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_pos1, batch_pos2, ins_label, scope, ent1_ids, ent2_ids, marks,omit_types,sent_ids)
...
def train(data, save_model_dir):
    ...
    if data.trigger_train and data.punishment_train == False:
        print("trigger loss!!!")
    if data.punishment_train and data.trigger_train == False:
        print("punishment train !!!")
    if data.trigger_train and data.punishment_train:
        print("two punishment train!!!")
    for idx in range(data.HP_iteration):
        ...
        for batch in data.batch_iter('train',batch_size):
            gaz_list, batch_word, batch_biword, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_pos1, batch_pos2, ins_label, batch_label, mask, scope,trigger_ids, ent1_ids, ent2_ids,marks,omit_types,sent_ids = batch
            prob, alpha = model(gaz_list, batch_word, batch_biword, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_pos1, batch_pos2, ins_label, scope, ent1_ids, ent2_ids, marks, omit_types,sent_ids) # batch_size*num_classes

            if data.trigger_train:
                trigger_ids_softmax = torch.nn.functional.softmax(trigger_ids, dim=1).float()
                loss_kl_sum = loss_kl(torch.log(alpha), trigger_ids_softmax) / batch_size

            if data.punishment_train:
                punishment_loss =torch.mean(torch.nn.functional.relu(alpha - data.ma_ga), dim=1)

            if data.trigger_train and data.punishment_train==False:
                batch_loss = loss_fn(prob, batch_label) + loss_kl_sum

            elif data.punishment_train and data.trigger_train==False:
                batch_loss =loss_fn(prob, batch_label)+ punishment_loss

            elif data.punishment_train and data.trigger_train:
                batch_loss = loss_fn(prob, batch_label) + loss_kl_sum + punishment_loss
            else:
